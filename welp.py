...
-------------------------------------------------------------------------------
 ...
 4 - Count lines in log file then give status every 1% completed - use http://pypi.python.org/pypi/progressbar
 ...
 12- Mod the IP field to also allow for domain names
    12b - Do reverse lookup of the domain names to get IPs and stats
...
def main():
    ...
    if line_counter == 1:
        ...
        rematch(log_file[0])
        print bcolors.BLUE + "[info] " + bcolors.ENDC + "Log format found to be %s" % log['type']

    # Actually start to look for stuff
    print bcolors.BLUE + "[info] " + bcolors.ENDC + "Analyzing the file: ", user_log_file
    print bcolors.BLUE + "[info] " + bcolors.ENDC + "Processing line ",
    ...
    # Show the Results
    if len(attacker) == 0:
        print bcolors.BLUE + "[info] " + bcolors.ENDC + "No strings found."
...
def findIt(line, line_counter, search_cat, search_strings):
    ...
    for id in php_ids_rules.keys():
        try:
            ...
            print "[Error] Compiling PHP-IDS rule %s failed. Skipping it." % id
            continue
...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        sys.exit('\n[Error] Fatal Error. You need to enter in the full\
    ...
    try:
        ...
        sys.exit("\n[Error] Can't read file the logfile you entered.\
                  \n[BuhBye]\n")
    ...
    try:
        ...
        sys.exit("\n[Error] Can't read file the PHP-IDS default_filter.xml.\
                  \n     Please get the latest file from https://phpids.org/\
                  \n     and place the XML file in the same directory as this script.\
                  \n[BuhBye]\n")

    # Cycle through all the PHP-IDS regexs and make a dictionary
    print "[info] Opened the PHP-IDS filter file and parsing the rules. "
    for filt in xmldoc.getElementsByTagName('filter'):
        id_xml = filt.getElementsByTagName('id')[0].toxml()
        id_content = id_xml.replace('<id>','').replace('</id>','')
        rule_xml = filt.getElementsByTagName('rule')[0].toxml()
        ...
        try:
            ...
            print "[Error] Compiling PHP-IDS rule %s failed. Skipping it." % id_content
            continue
    ...
    # Using line 1 - see what kind of log this is
    if line_counter == 1:
        print "[info] Examining the log format"
        rematch(log_file[0])
        print "[info] Log format found to be %s" % log['type']

    # Actually start to look for stuff
    print "[info] Analyzing the file: ", user_log_file
    print "[info] Processing line ",
    ...
    # Show the Results
    if len(attacker) == 0:
        print "[info] No strings found."

    elif len(attacker) > 0:
        print "[FOUND] Found the following IPs (and associated activity)"

        attacker.sort(key=operator.itemgetter('string'))
        for event in attacker:
            print '    [FOUND] "%s" found in IP: %s in line# %d -> %s' % (event['string'], event['ip'], event['line_number'], event['line']        )
